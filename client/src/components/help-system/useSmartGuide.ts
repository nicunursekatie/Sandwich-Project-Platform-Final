...
export function useSmartGuide(): UseSmartGuideReturn {
  ...
  const [strugglingWithFeature, setStruggleFeature] = useState<string | undefined>();
  ...
  const setStruggleFeatureWrapper = useCallback((feature: string | undefined) => {
  ...
  const getPersonalizedTips = useCallback((): string[] => {
    ...
    // Activity-based tips
    const collectionActivities = recentActivity.filter(a => a.includes('collection'));
    if (collectionActivities.length > 10) {
      tips.push('You\'re a collection pro! Try keyboard shortcuts: Ctrl+N for new collection');
    }
