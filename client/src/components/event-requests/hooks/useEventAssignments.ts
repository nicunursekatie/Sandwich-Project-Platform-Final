  ...
  const resolveUserName = (userIdOrName: string | undefined): string => {
    ...
    try {
      ...
      if (userIdOrName.includes('_') && (userIdOrName.startsWith('user_') || userIdOrName.startsWith('admin_') || userIdOrName.startsWith('committee_'))) {
        ...
        logger.debug(`TSP Contact user not found: ${userIdOrName}`);
      ...
      if (userIdOrName.startsWith('volunteer-')) {
        ...
        logger.debug(`Volunteer not found: ${userIdOrName}`);
      ...
      if (/^\d+$/.test(userIdOrName)) {
        ...
        logger.debug(`Person not found in resolveUserName: ID=${userIdOrName}`);
