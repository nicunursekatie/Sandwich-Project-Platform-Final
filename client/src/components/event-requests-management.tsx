...
export default function EventRequestsManagement({
  ...
  const canSelfSignup = (eventRequest: EventRequest, type: 'driver' | 'speaker' | 'volunteer'): boolean => {
    ...
    if (type === 'driver') {
      ...
      // ALWAYS allow volunteer signup for scheduled events, regardless of volunteersNeeded
      ...
      // For non-scheduled events, check volunteersNeeded
      if (eventRequest.volunteersNeeded && eventRequest.volunteersNeeded > 0) {
        const currentVolunteers = parsePostgresArray(eventRequest.assignedVolunteerIds);
        ...
        try {
          ...
          if (request.assignedDriverIds) {
            ...
            for (const driverId of driverIds) {
              // Handle both driver IDs (numeric) and driver names (strings)
              // Some legacy data has driver names instead of IDs
              
              // First, check if it's a driver name stored directly
              if (typeof driverId === 'string' && driverId.toLowerCase().includes(searchLower)) {
                return true;
              }
              
              // Then try to find the driver by ID in the drivers list
              ...
              // Also check if the driverId itself contains driver name (for legacy data like "John Stembel")
              const driverIdStr = String(driverId);
              if (driverIdStr.toLowerCase().includes(searchLower)) {
                return true;
              }
            }
          }
