...
import { DynamicErrorManager, DynamicErrorMessage, ErrorContext } from '@shared/error-management';
...
export function useErrorHandler() {
  const [currentError, setCurrentError] = useState<DynamicErrorMessage | null>(null);
  const { user } = useAuth();
  ...
  const handleError = useCallback((
    ...
    const errorMessage = DynamicErrorManager.getErrorMessage(error, enhancedContext);
    setCurrentError(errorMessage);

    // Show toast notification for user-friendly errors
    if (showToast && errorMessage.severity !== 'low') {
      const toastVariant = errorMessage.severity === 'critical' ? 'destructive' : 'default';
      
      toast({
        variant: toastVariant,
        title: `${DynamicErrorManager.getCategoryIcon(errorMessage.category)} ${errorMessage.title}`,
  ...
  const handleFormError = useCallback((error: Error | string, formData?: Record<string, any>) => {
    handleError(error, {
      context: {
  ...
  const handleErrorAction = useCallback((action: any) => {
    ...
    switch (action.target) {
      ...
      case 'resetForm':
        ...
        toast({
          title: "Form Reset",
          description: "The form has been cleared and reset to default values."
        });
        break;
      ...
      case 'saveLocal':
        try {
          ...
          if (Object.keys(formData).length > 0) {
            ...
            toast({
              title: "Data Saved",
              description: "Your form data has been saved locally as a backup."
            });
          } else {
            toast({
        ...
        fetch('/api/health-check')
          ...
          .catch(() => {
            toast({
              variant: "destructive",
              title: "Network Issue",
              description: "Network connection appears to be having problems."
            });
          });
        break;
      ...
      case 'showAlternatives':
        toast({
          ...
          description: "Try using a different feature or contact support for assistance."
        });
        break;
      ...
      case 'checkServiceStatus':
        ...
        toast({
          ...
          description: "Checking external service status... Please try again in a few minutes."
        });
        break;
      ...
      case 'scheduleRetry':
        ...
        toast({
          ...
          description: "We'll automatically retry this operation in 1 minute."
        });
        break;
...
export function createFormErrorHandler(handleError: (error: Error | string, options?: ErrorHandlerOptions) => void) {
