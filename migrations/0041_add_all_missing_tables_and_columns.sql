...
-- Add unique constraint (event_request_id, tsp_contact_user_id, reminder_type) if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'idx_tsp_followup_unique'
      AND table_name = 'tsp_contact_followups'
  ) THEN
    CREATE UNIQUE INDEX idx_tsp_followup_unique
