dimport { pgTable, serial, text, boolean, timestamp, integer, unique, varchar, jsonb, index, foreignKey, numeric, time, primaryKey } from "drizzle-orm/pg-core"
...
export const userActivityLogs = pgTable("user_activity_logs", {
	...
	index("idx_user_activity_section_time").using("btree", table.section.asc().nullsLast().op("timestamp_ops"), table.createdAt.asc().nullsLast().op("text_ops")),
	...
	index("idx_user_activity_user_time").using("btree", table.userId.asc().nullsLast().op("timestamp_ops"), table.createdAt.asc().nullsLast().op("timestamp_ops")),
]);
