...
export class DynamicErrorManager {
  private static errorMappings: Map<string, (context: ErrorContext) => DynamicErrorMessage> = new Map();
  ...
  static {
    ...
    this.registerError('AUTH_EXPIRED', (context) => ({
      ...
      severity: 'medium',
      userFriendlyExplanation: 'You need to sign in again to continue using the platform.',
      recoveryActions: [
        { label: 'Sign In Again', action: 'redirect', target: '/landing', primary: true },
    ...
    this.registerError('AUTH_INVALID', (context) => ({
      ...
      category: 'authentication',
      severity: 'medium',
      userFriendlyExplanation: 'Please double-check your email address and password.',
      recoveryActions: [
        { label: 'Try Again', action: 'retry', primary: true },
        { label: 'Reset Password', action: 'redirect', target: '/reset-password' },
    ...
    this.registerError('PERMISSION_DENIED', (context) => ({
      ...
      severity: 'medium',
      userFriendlyExplanation: 'This feature is restricted to users with higher access levels.',
      recoveryActions: [
        { label: 'Go to Dashboard', action: 'redirect', target: '/dashboard', primary: true },
    ...
    this.registerError('NETWORK_ERROR', (context) => ({
      ...
      category: 'network',
      severity: 'high',
      userFriendlyExplanation: 'There seems to be a problem with your internet connection or our servers.',
      recoveryActions: [
        { label: 'Try Again', action: 'retry', primary: true },
        { label: 'Refresh Page', action: 'refresh' },
        { label: 'Check Network Status', action: 'custom', target: 'checkNetworkStatus' }
      ],
      preventionTips: [
    ...
    this.registerError('VALIDATION_ERROR', (context) => ({
      ...
      category: 'validation',
      severity: 'low',
      userFriendlyExplanation: 'Some required information is missing or incorrectly formatted.',
      recoveryActions: [
        { label: 'Review Form', action: 'custom', target: 'scrollToErrors', primary: true },
        { label: 'Clear Form & Start Over', action: 'custom', target: 'resetForm' },
    ...
    this.registerError('DATABASE_ERROR', (context) => ({
      ...
      category: 'database',
      severity: 'high',
      userFriendlyExplanation: 'There was a temporary problem saving your data. Your work has not been lost.',
      recoveryActions: [
    ...
    this.registerError('FILE_UPLOAD_ERROR', (context) => ({
      ...
      severity: 'medium',
      userFriendlyExplanation: 'There was a problem uploading your file. This could be due to file size, type, or connection issues.',
      recoveryActions: [
        { label: 'Try Upload Again', action: 'retry', primary: true },
        { label: 'Choose Different File', action: 'custom', target: 'selectNewFile' },
        { label: 'Check File Requirements', action: 'custom', target: 'showFileHelp' }
      ],
      preventionTips: [
    ...
    this.registerError('DATA_LOADING_ERROR', (context) => ({
      ...
      severity: 'medium',
      userFriendlyExplanation: 'The information you requested is temporarily unavailable.',
      recoveryActions: [
    ...
    this.registerError('EXTERNAL_SERVICE_ERROR', (context) => ({
      ...
      severity: 'high',
      userFriendlyExplanation: 'We use external services for some features. One of these services is temporarily down.',
      recoveryActions: [
        { label: 'Try Again Later', action: 'custom', target: 'scheduleRetry', primary: true },
        { label: 'Use Alternative Method', action: 'custom', target: 'showAlternatives' },
        { label: 'Check Service Status', action: 'custom', target: 'checkServiceStatus' }
      ],
      preventionTips: [
  ...
  static registerError(errorCode: string, errorGenerator: (context: ErrorContext) => DynamicErrorMessage): void {
    this.errorMappings.set(errorCode, errorGenerator);
  }

  static getErrorMessage(error: Error | string, context: ErrorContext = {}): DynamicErrorMessage {
    const errorCode = typeof error === 'string' ? error : this.extractErrorCode(error);
  ...
  private static extractErrorCode(error: Error): string {
    ...
    if (error.message.includes('401') || error.message.includes('Unauthorized')) return 'AUTH_EXPIRED';
    ...
    if (error.message.includes('validation') || error.message.includes('required')) return 'VALIDATION_ERROR';
    if (error.message.includes('database') || error.message.includes('query')) return 'DATABASE_ERROR';
    if (error.message.includes('file') || error.message.includes('upload')) return 'FILE_UPLOAD_ERROR';
    
    return 'UNKNOWN_ERROR';
  }

  private static createGenericError(error: Error | string, context: ErrorContext): DynamicErrorMessage {
    ...
    return {
      ...
      category: 'unknown',
      severity: 'medium',
      userFriendlyExplanation: 'We encountered an unexpected problem. This has been logged and our team will investigate.',
      recoveryActions: [
        { label: 'Try Again', action: 'retry', primary: true },
        { label: 'Refresh Page', action: 'refresh' },
