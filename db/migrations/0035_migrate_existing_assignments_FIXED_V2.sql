-- Migration: 0035_migrate_existing_assignments_FIXED_V2.sql
-- Purpose: Migrate existing assignment data to new junction tables
-- Date: 2025-11-15
-- Breaking: No (dual-write period - keeps old columns intact)
-- FIXED V2: Use correct users table columns (display_name, first_name, last_name instead of full_name)
...
-- FIXED: Use display_name, or concat first_name + last_name, or 'Unknown User'
WITH multi_assignees AS (
  SELECT
    p.id as project_id,
    jsonb_array_elements_text(p.assignee_ids) as user_id,
    p.created_at
  FROM projects p
...
WITH support_people AS (
  SELECT
    p.id as project_id,
    jsonb_array_elements_text(p.support_people_ids) as user_id,
    p.created_at
  FROM projects p
...
WITH team_board_assignees AS (
  SELECT
    tbi.id as item_id,
    unnest(tbi.assigned_to) as user_id,
    tbi.created_at
