...
UPDATE projects
SET
  owner_id = assignee_id::TEXT,  -- Cast to TEXT just in case
...
SELECT
  id as project_id,
  assignee_id::TEXT as user_id,  -- Cast to TEXT
...
-- FIXED: Removed ::INTEGER cast, keep as TEXT
WITH multi_assignees AS (
  SELECT
    p.id as project_id,
    jsonb_array_elements_text(p.assignee_ids) as user_id,  -- Keep as TEXT, don't cast to INTEGER
    p.created_at
  FROM projects p
...
LEFT JOIN users u ON u.id = ma.user_id  -- Now both are TEXT, no cast needed
ON CONFLICT (project_id, user_id) DO NOTHING;
...
WITH support_people AS (
  SELECT
    p.id as project_id,
    jsonb_array_elements_text(p.support_people_ids) as user_id,  -- Keep as TEXT
    p.created_at
  FROM projects p
...
LEFT JOIN users u ON u.id = sp.user_id  -- Now both are TEXT
ON CONFLICT (project_id, user_id) DO NOTHING;
...
-- FIXED: No cast needed, assignee_id is already TEXT
INSERT INTO task_assignments (task_id, user_id, user_name, role, added_at)
SELECT
  id as task_id,
  assignee_id as user_id,  -- Already TEXT, no cast needed
  assignee_name as user_name,
  'assignee' as role,
  created_at as added_at
FROM project_tasks
...
WITH task_multi_assignees AS (
  SELECT
    pt.id as task_id,
    unnest(pt.assignee_ids) as user_id,  -- Keep as TEXT, don't cast
    pt.created_at
...
LEFT JOIN users u ON u.id = tma.user_id  -- Now both are TEXT
ON CONFLICT (task_id, user_id) DO NOTHING;
...
WITH team_board_assignees AS (
  SELECT
    tbi.id as item_id,
    unnest(tbi.assigned_to) as user_id,  -- Keep as TEXT, don't cast
    tbi.created_at
...
LEFT JOIN users u ON u.id = tba.user_id  -- Now both are TEXT
