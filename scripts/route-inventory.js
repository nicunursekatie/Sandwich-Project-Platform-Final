...
function occurrencesSummary(occurrences, limit = 4) {
  // Show DISTINCT FILES only — deliberately no `:line` suffix. Line numbers
  // shift on almost every unrelated edit, and because this report is committed
  // to the repo, those shifts caused perpetual merge conflicts in
  // docs/route-inventory.md (every branch regenerated different line numbers).
  // Emitting just the file paths makes the committed artifact stable: it only
  // changes when the route SET itself changes, not when code moves around.
  // `--check` already normalized line numbers away, so this doesn't weaken
  // drift detection — it just stops the doc from being a merge-conflict magnet.
  // Files are sorted for deterministic output independent of filesystem walk
  // order (keeps --check stable across machines).
  const distinctFiles = [...new Set(occurrences.map((occ) => occ.file))].sort(byString);
  return distinctFiles.slice(0, limit).join('<br>');
}
...
function buildReport({ apiRefs, apiRequestMismatches, serverRoutes, appRoutes, unmatched }) {
  ...
  // File only (no `:line`) — same rationale as occurrencesSummary: keep the
  // committed report stable against line-number churn.
  const mismatchRows = apiRequestMismatches.map((item) => [item.url, item.file]);
...
// The report no longer emits `:line` suffixes (see occurrencesSummary), so the
// committed doc only changes on genuine route drift — a new/removed/renamed API
// path or mount. This normalization is kept as a defensive safety net so any
// stray line number that slips into the report can't make --check red on
// cosmetic churn.
