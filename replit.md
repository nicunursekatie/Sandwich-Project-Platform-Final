...
Event Request Card Design: Scheduled event cards (client/src/components/event-requests/cards/ScheduledCard.tsx) use gradient background (from-[#fef3e2] via-[#FBAD3F]/60 to-[#FBAD3F]/40) with colored section boxes for visual organization. Improved typography with text-xl sm:text-2xl (instead of text-5xl) and font-bold (instead of font-black) for better readability. Sections use distinct colors: Contact Info (#236383), Event Details (#47B3CB), Delivery & Logistics (#A31C41), Team Assignments (#47B3CB), Notes (#47B3CB). Oct 28, 2025 typography improvements for professional appearance while maintaining original design aesthetic.
