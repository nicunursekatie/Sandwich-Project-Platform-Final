...
**Projects Disconnected from Google Sheets Sync (Nov 14, 2025)**: Permanently disabled Google Sheets synchronization for the projects table to prevent sync conflicts and data integrity issues. **Decision**: After persistent issues with archived projects being recreated by background sync, the decision was made to manage projects entirely within the application database. **Implementation**: Disabled the `syncProjects()` call in `server/background-sync-service.ts` (line 100) - projects are no longer synced to or from Google Sheets during the 5-minute background sync cycle. **Impact**: (1) Projects stay stable - no more archive/recreate cycles, (2) No more orphaned meeting notes from projects getting new IDs, (3) Project management is now completely app-based and reliable, (4) Event requests sync continues to work normally. **What Still Syncs**: Event requests continue to sync from Google Sheets every 5 minutes, and past scheduled events auto-transition to completed status as expected. **Location**: `server/background-sync-service.ts` (lines 98-100).
...
-   **Google Sheets Integration**: Automatic synchronization for event requests only (projects are managed in-app).
