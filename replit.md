...
**Postponed Events Workflow (Nov 18, 2025)**: Added dedicated workflow for postponed events to prevent them from falling off the radar while avoiding false overdue flags. Events that need rescheduling at an undefined future date now have their own "Postponed" tab (separate from Declined/Cancelled) with tracking for postponement reason, tentative new date, and situational notes. Solution: (1) Added three new optional fields to event_requests schema: `postponement_reason` (TEXT, required when postponing), `tentative_new_date` (TIMESTAMP, optional), `postponement_notes` (TEXT, optional), (2) Created PostponedCard component with amber color scheme to display postponement details, (3) Built PostponementDialog for capturing postponement information with form validation, (4) Added POST `/api/event-requests/:id/postpone` backend endpoint with proper camelCase-to-snake_case field mapping (postponementReason → postponement_reason, tentativeNewDate → tentative_new_date, postponementNotes → postponement_notes), (5) Updated RequestFilters to show separate "Postponed" tab with Pause icon, (6) Modified DeclinedTab to exclude postponed events (they now appear only in PostponedTab), (7) Implemented proper React Query cache invalidation using apiRequest and queryClient.invalidateQueries. **Technical Pattern**: Zod schema extensions ensure type safety across frontend/backend, proper snake_case mapping prevents data loss, and React Query mutation patterns ensure UI stays synchronized. **Locations**: `shared/schema.ts`, `client/src/components/event-requests/tabs/PostponedTab.tsx`, `client/src/components/event-requests/cards/PostponedCard.tsx`, `client/src/components/event-requests/dialogs/PostponementDialog.tsx`, `server/routes/event-requests.ts`.
