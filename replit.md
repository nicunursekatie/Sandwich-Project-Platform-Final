...
- **Task Creation from Meeting Notes**: Implemented functionality to automatically convert meeting discussion notes and decision items into actionable project tasks via the "Create Tasks from Notes" button, with appropriate priority levels and due dates. **TECHNICAL FIX DOCUMENTED**: Resolved "phantom task" issue where tasks appeared incomplete - root cause was projects lacking meeting discussion points in database. Task creation works correctly when `meetingDiscussionPoints` field contains data. Field mapping between frontend camelCase (`meetingDiscussionPoints`) and database snake_case (`meeting_discussion_points`) functions properly via Drizzle ORM.
...
- **Permission Name Migration Issues**: **TECHNICAL FIX DOCUMENTED**: Hosts and Recipients showing 0 items despite proper user permissions caused by API endpoints using old permission names (`access_hosts`, `access_recipients`) instead of new permission constants (`HOSTS_VIEW`, `RECIPIENTS_VIEW`). Fixed in `server/routes.ts` by updating all host/recipient API endpoints and in `client/src/pages/dashboard.tsx` by updating navigation permission checks to use correct PERMISSIONS constants. Affects routes: `/api/hosts/*`, `/api/recipients`, `/api/hosts-with-contacts`, export endpoints, and dashboard navigation.
- **Email Field Cache Invalidation Fix**: **TECHNICAL FIX DOCUMENTED**: Email addresses were saving correctly to database via inline editing but not appearing in edit details forms due to stale React Query cache. Root cause: `detailsRequest` state captured at dialog open time became stale after inline edits. Fixed in `client/src/components/event-requests-management.tsx` by: (1) Adding intelligent cache refresh logic in `updateMutation.onSuccess` that refetches data when edit details dialog is open for the same event, (2) Implementing proper form re-rendering with dynamic React keys, (3) Using `queryClient.refetchQueries()` instead of setTimeout for reliable data refresh. Inline editing now properly updates edit details forms immediately.
- **Number Input Scroll Behavior Fix**: **TECHNICAL FIX DOCUMENTED**: Resolved terrible UX issue where users attempting to scroll while focused on number input fields (sandwich count, drivers needed, etc.) would accidentally change values instead of scrolling the page, causing data corruption and frustrating users who thought scroll was broken. Root cause: HTML5 number inputs capture wheel events to increment/decrement values by default. Fixed in `client/src/components/event-requests-management.tsx` by adding `onWheel={(e) => e.target.blur()}` handlers to all 9 number input fields, causing inputs to lose focus when users scroll and allowing normal page scrolling behavior. Eliminates accidental data changes while preserving keyboard and direct click input functionality.
- **Google Sheets Authentication Node.js v20+ Compatibility Issue**: **TECHNICAL FIX DOCUMENTED**: "Invalid JWT Signature" errors occur when using service account keys generated with older OpenSSL standards that Node.js v20+ rejects. Both GoogleAuth and file-based authentication methods fail identically. Root cause: Service account keys created before Node.js v20 compatibility may use crypto standards that current Node.js versions reject. Solution: Delete existing service account keys in Google Cloud Console and generate fresh ones. The issue persists across multiple authentication methods, confirming it's a key compatibility issue rather than code implementation. Key format processing works correctly (proper 28-line PEM format) but JWT signature validation fails at Google's servers.
