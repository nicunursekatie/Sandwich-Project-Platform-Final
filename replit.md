...
DateTimePicker Timezone Fix (Nov 7, 2025): Fixed critical timezone conversion bug where pickup times entered in the DateTimePicker (e.g., 9:30 PM) were being saved incorrectly (e.g., 4:30 PM). **Root Cause**: Both frontend and backend were calling `.toISOString()` which converts local time to UTC, causing a 5-hour shift for EST/EDT users. When UTC time was stored in the database `timestamp without time zone` column and read back, it was interpreted as local time, creating the incorrect shift. **Solution**: Modified both frontend DateTimePicker and backend conversion functions to construct local datetime strings (`YYYY-MM-DDTHH:MM:SS`) directly without timezone conversion, ensuring the time entered by the user is exactly what gets stored and displayed. Location: `client/src/components/ui/datetime-picker.tsx` lines 72-96, `server/routes/event-requests.ts` lines 30-59, `server/routes/import-events.ts` lines 18-48.
