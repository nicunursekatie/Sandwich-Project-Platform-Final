...
Event Request Status Colors Vibrant Update (Nov 3, 2025): Completely revamped event request card status colors from muted/gloomy to bright and vibrant for maximum visibility. Location: `client/src/components/event-requests/constants.ts`. New colors: New=#00C9FF (bright cyan), In Process=#FF9500 (bright orange), Scheduled=#007AFF (bright blue), Completed=#34C759 (bright green), Declined=#FF3B30 (bright red). Also increased border width to 2px, added shadows, and doubled background tint opacity for better visual presence. **Card Background Update**: Changed all event request card backgrounds from plain white or muted gradients (15-20% opacity) to vibrant color gradients (20% → 10% → white) with thicker left borders (6px instead of 4px) and enhanced shadows. Files updated: `NewRequestCard.tsx`, `InProcessCard.tsx`, `ScheduledCard.tsx`, `ScheduledCardEnhanced.tsx`, `CompletedCard.tsx`, `DeclinedCard.tsx`.
