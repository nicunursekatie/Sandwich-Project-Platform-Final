...
- **Database Configuration**: Centralized database URL selection. Neon serverless requires `--> statement-breakpoint` markers for multi-statement SQL migrations and avoids `.returning()` on update operations. IMPORTANT: When using `db.execute(sql\`...\`)` with Neon serverless driver, the result is returned as an array directly, NOT as `{ rows: [...] }`. Always handle both formats: `const rows = Array.isArray(result) ? result : (result.rows || []);`
