...
Analytics Pages Updated (Oct 30, 2025): Both grant-metrics.tsx and impact-dashboard.tsx now fetch and use hybrid stats for yearly totals instead of calculating from collection log. This fixes the 265k+ missing sandwiches issue in analytics displays. Pages fetch `/api/collections/hybrid-stats` and use the `byYear` object for authoritative yearly sandwich counts (2020-2025). Fallback to collection log calculation only if hybrid stats unavailable.
