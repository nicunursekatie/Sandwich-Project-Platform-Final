...
- **Stale Event Escalation Email Batching**: Escalation emails for stale events (2+ weeks no contact) are now batched into ONE weekly summary email to Katie & Christine instead of individual emails per event. The `sendBatchedStaleEventEscalation` function groups all stale events by TSP contact and sends a single digest email. Rate limiting via `adminEscalationSentAt` field prevents the same events from being included more than once per week. Toolkit follow-up emails now say "you sent the toolkit" for personalization.
