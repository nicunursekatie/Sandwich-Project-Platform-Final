...
- **Event Request Intake Validation System**: Comprehensive validation badges that flag incomplete intake information on in-process and scheduled event cards. Uses shared `getMissingIntakeInfo` utility function to check for missing contact info (email OR phone), sandwich count (estimatedSandwichCount - types are optional nice-to-have), and address (eventAddress, deliveryDestination, or overnightHoldingLocation) ONLY when TSP drivers/van driver are needed. Driver-related fields (pickup contact person, pickup time window) are NOT validated at intake since those are staffing concerns handled in the staffing planning dialog after the event is scheduled. Address validation is skipped when organization is delivering themselves (driversNeeded = 0 and vanDriverNeeded = false) to prevent alarm fatigue. Individual badges clearly list each missing item (e.g., "Missing: Contact Info", "Missing: Sandwich Info", "Missing: Address") for immediate visibility across all devices. Includes a centralized Missing Info Summary Dialog accessible via header button that displays all events with incomplete intake information, with filters for status (Both/In Process/Scheduled) and date range (Next 30 Days/Next 60 Days/All Upcoming), defaults to showing next 30 days. Lists what's missing from each event (including department name when assigned) sorted by event date (soonest first) with click-to-navigate functionality (scrolls to and highlights the specific event card). Event cards have unique IDs (`event-card-${id}`) to enable smooth scroll navigation from the summary. Prevents events from "falling through the cracks" by ensuring critical planning information is complete.
- **Team Board Commenting System**: Full-featured commenting system on all team board items (tasks, notes, ideas, reminders). Each item displays a real-time comment count badge that's fetched immediately on page load (no need to expand). Users can expand any item to view existing comments and post new ones. Comments include author name, timestamp, and content. Only comment authors can delete their own comments (with confirmation prompt). Backend uses `team_board_comments` table with proper foreign key relationships and cascade delete. Frontend uses the ItemComments component which accepts an `initialCommentCount` prop to display counts immediately. Both create and delete mutations invalidate the main team board query to ensure comment counts update in real-time after any changes. Comment aggregation uses `inArray` and `count` from drizzle-orm for efficient bulk fetching of comment counts across all items.
- **HTML Email Rendering Fix**: Fixed SendGrid email delivery to properly render HTML templates. Previously, when both HTML and plain text versions were sent, some email clients (like Gmail) would default to showing the plain text version instead of the branded HTML template. Modified `server/sendgrid.ts` to prioritize HTML content and only include plain text as a fallback when genuinely different from HTML. This ensures toolkit emails display with full branding, colors, and styling as designed.
- **User Management Dialog Scope**: The User Management dialog is streamlined to focus on user administration only, containing three tabs: Profile (basic info and role), Permissions (permission management), and Activity (user activity history). All feature pages (Event Requests, My Availability, Team Availability, Volunteer Calendar, Team Board, Host Locations Map, and Coolers) were mistakenly embedded as tabs in this dialog and have been corrected to exist only as standalone pages accessible through the main dashboard navigation. Each feature page is controlled by its respective `NAV_*` permission.
- **Official Sheet Tracking**: Event requests now include an `addedToOfficialSheet` boolean field to track which scheduled events have been manually added to the official Google Sheet (separate from the automated sync sheet). Scheduled events display a clickable toggle badge that changes from gray "Not on Official Sheet" to blue "✓ On Official Sheet" instantly when clicked, using the same quick-toggle pattern as the date confirmation badge. This helps staff track which events still need to be manually added to the separate official events spreadsheet.
