...
**Kudos Real-Time Popup Notifications Implemented (Nov 13, 2025)**: Replaced login-only kudos notifications with real-time popup system. **User Requirement**: Users shouldn't have to visit a separate kudos inbox - popup notifications should appear immediately when kudos are received. **Implementation**: Created `RealTimeKudosNotifier` component that polls `/api/messaging/kudos/unnotified` every 30 seconds and displays toast popups (max 3 at a time, staggered by 800ms). Removed "Kudos" navigation link from sidebar - the archive page still exists at `#/dashboard?section=kudos` but is hidden from main navigation and accessible via "View Archive" button in toast. Replaced the old `KudosLoginNotifier` (which only showed notifications on login) with the new real-time version to prevent duplicate notifications. **Architecture**: Uses TanStack Query polling (30s interval) instead of Socket.IO for simplicity. Tracks shown kudos IDs in component state to prevent duplicate toasts. Marks kudos as "initially notified" in database after display. **UX Impact**: Users now get immediate visual feedback when they receive kudos without needing to manually check an inbox. The archive remains available but is not part of regular navigation flow. Location: `client/src/components/real-time-kudos-notifier.tsx`.

**Collections Log Group Sandwich Calculation Bug Fixed (Nov 13, 2025)**: Fixed filtered stats showing incorrect group sandwich counts when filtering Collections Log by date range. **Problem**: When filtering Collections Log for January 2025, the stats card displayed "490 GROUPS" instead of the expected ~20,000+ sandwiches (including a single collection with 19,706 group sandwiches on Jan 20). **Root Cause**: The inline group sandwich calculation in the queryFn (lines 567-572 of `sandwich-collection-log.tsx`) only counted sandwiches from the new `groupCollections` JSONB format but completely ignored the legacy `group1_count` and `group2_count` database columns. The problematic collection stored its 19,706 sandwiches in `group1_count` with an empty `groupCollections` array, so they were never counted. **Solution**: Replaced the inline calculation with a call to `calculateGroupTotal()` (which wraps the standardized `calculateGroupSandwiches()` function from `analytics-utils.ts`). This function properly handles BOTH the new JSONB format AND the legacy columns, ensuring all group sandwiches are counted regardless of storage format. **Impact**: Critical data accuracy issue resolved - filtered statistics now correctly display total group sandwiches including legacy data. Location: `client/src/components/sandwich-collection-log.tsx` (line 570).
