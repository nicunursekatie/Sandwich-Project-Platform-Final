...
- **Legacy local-file download/preview 500 (prod-only) — fixed**: older documents have legacy local `filePath`s (e.g. `uploads/documents/...`) that take the disk branch in `server/routes/documents.ts`. That branch called `require('fs')` inline, but the project is ESM (`"type": "module"`) and the prod build bundles with `esbuild --format=esm` with NO `createRequire` banner, so `require` is undefined at runtime → threw → outer catch returned generic 500 `{"error":"Failed to download document"}`. Dev hid it because `tsx` shims `require` (returned the clean 404 instead). **Rule: never use `require(...)` in server code — it only works in dev under tsx and throws in the prod ESM bundle; use top-level ESM `import` instead.** The same `require()` landmine was swept from the whole server: `server/storage.ts` (populate-from-public-folders), `server/routes/storage/index.ts` (admin populate handler), and `server/monitoring/health-checks.ts` (disk check) were all converted to top-level imports — `rg "require\(" server/` is now clean. Local file streams now also have `.on('error')` handlers. Requires republish to fix prod.
...
- **Rule: create group chats with an EXPLICIT channel ID, never `undefined`.** A `messaging` channel created with no ID + a member list becomes a Stream "distinct" channel (identity = member set), which rejects `addMembers`/`removeMembers` with error 17 ("cannot add members to the distinct channel"). In `server/routes/stream.ts` `POST /channels`, groups (3+ members or any `channelName`) get `group_<random>` IDs; only 1:1 DMs stay distinct (no ID) so the same pair reuses one channel. Groups created before this fix are permanently distinct and cannot be edited — the `POST /channels/:type/:id/members` handler returns a plain-language "start a new group" message for Stream error 17 instead of the raw error. (`client/src/components/group-messaging.tsx` is the separate legacy DB conversation system — unrelated.)

**Traffic Conflict Badges (World Cup / Atlanta events):**
- Known high-traffic dates live in `shared/traffic-conflicts.ts` (`TRAFFIC_CONFLICTS`, YYYY-MM-DD Eastern). `getTrafficConflictForEvent(...dates)` returns a hit if ANY supplied date matches. **Rule: on event cards, pass only the operative date — `[request.scheduledEventDate ?? request.desiredEventDate]`, NOT both.** Passing both left a stale badge when an event was rescheduled OFF a conflict date (e.g. requested July 15 = real Atlanta WC semifinal, rescheduled to July 27) because the old desiredEventDate still matched. Fixed in `ScheduledCardEnhanced`, `NewRequestCard`, `InProcessCard`. (Atlanta 2026 WC semifinal is genuinely July 15; final July 19.)

**UI — Floating Help Button vs Chat Send Button:**
- The `GuidedTour` floating help button (`fixed bottom-4 right-4`) is hidden on sections with a bottom-anchored send/reply composer (`HELP_BUTTON_HIDDEN_SECTIONS` in `client/src/pages/dashboard.tsx`: `chat`, `messages`, `inbox`, `stream-messages`, `gmail-inbox`) so it doesn't cover the Team Chat / Stream Chat send button. **Rule: `GuidedTourGate` must check the dashboard's real `activeSection` AND the multi-view panel sections — in single-view navigation the panel state lags behind `activeSection`, so checking panels alone left the button visible over Team Chat.**
