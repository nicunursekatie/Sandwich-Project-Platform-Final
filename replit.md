...
**Contact Attempt User Attribution Fixed (Nov 18, 2025)**: Fixed critical issue where contact attempts on event cards were not showing WHO made the attempt. The problem was that when users completed contact details (`/details` endpoint) or recorded follow-ups (`/follow-up` endpoint), the code wasn't adding entries to the `contactAttemptsLog` array with user attribution. Solution: (1) Modified both endpoints to create contactAttemptsLog entries with `createdBy` (user ID) and `createdByName` (full name) fields, (2) Used a two-step update process to avoid race conditions where concurrent contact attempts could overwrite each other - first update basic fields, then fetch fresh data and append to the log, (3) Frontend already had the display code for user attribution, it just wasn't receiving the data. All new contact attempts now display the user's name in a prominent colored section below the attempt title. **Location**: `server/routes/event-requests.ts`.
