...
-   **Event Request Audit Logging Data Integrity Fix (Nov 2025)**: Fixed critical audit logging gap where event requests created through import paths (Google Sheets, Excel, scheduled events imports) were missing audit trail entries. ROOT CAUSE: Only the manual API creation endpoint (`POST /api/event-requests`) had audit logging - all import paths called `storage.createEventRequest()` directly without logging. This caused events imported with TSP contact assignments to appear in the system with no audit history. FIX: Added `AuditLogger.logEventRequestChange()` calls to all import paths: (1) Google Sheets import (`/import-events` - 4 routes), (2) Excel import service (`excel-import-service.ts`), (3) Scheduled events import (`import-scheduled-events.ts`). All imports now create audit logs with `userId: 'SYSTEM'`, descriptive `userAgent` values (e.g., "Excel - Historical 2024 Import"), and operation context (e.g., `operation: 'GOOGLE_SHEETS_IMPORT'`). This ensures complete audit trail for all event request creation paths, regardless of entry method.
