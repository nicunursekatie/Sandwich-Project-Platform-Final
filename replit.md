...
Event Request Status Colors Vibrant Update (Nov 3, 2025): Completely revamped event request card status colors from muted/gloomy to bright and vibrant for maximum visibility while maintaining TSP brand color palette (#236383, #FBAD3F, #007E8C, #A31C41, #47B3CB). Location: `client/src/components/event-requests/constants.ts`. The key change: DOUBLED the opacity intensity (15% → 30%) on card backgrounds to make colors pop. Also increased border width from 4px to 6px, added enhanced shadows (shadow-md with hover:shadow-xl), and created smooth color gradients (30% → 15% → white) for visual depth. Files updated: `NewRequestCard.tsx`, `InProcessCard.tsx`, `ScheduledCard.tsx`, `ScheduledCardEnhanced.tsx`, `CompletedCard.tsx`, `DeclinedCard.tsx`. Status mappings: New=#007E8C (teal), In Process=#FBAD3F (orange), Scheduled=#236383 (dark blue), Completed=#47B3CB (light blue), Declined=#A31C41 (red).
