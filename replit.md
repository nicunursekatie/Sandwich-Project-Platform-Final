...
- **Event Request Intake Validation System**: Comprehensive validation badges that flag incomplete intake information on in-process and scheduled event cards. Uses shared `getMissingIntakeInfo` utility function to check for missing contact info (email OR phone), sandwich count (estimatedSandwichCount - types are optional nice-to-have), and address (eventAddress, deliveryDestination, or overnightHoldingLocation). Driver-related fields (pickup contact person, pickup time window) are NOT validated at intake since those are staffing concerns handled in the staffing planning dialog after the event is scheduled. Individual badges clearly list each missing item (e.g., "Missing: Contact Info", "Missing: Sandwich Info", "Missing: Address") for immediate visibility across all devices. Includes a centralized Missing Info Summary Dialog accessible via header button that displays all events with incomplete intake information, listing what's missing from each event (including department name when assigned) with click-to-navigate functionality (scrolls to and highlights the specific event card). Event cards have unique IDs (`event-card-${id}`) to enable smooth scroll navigation from the summary. Prevents events from "falling through the cracks" by ensuring critical planning information is complete.
