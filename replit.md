...
Dual Data Architecture (Oct 30, 2025): System uses TWO separate tables for different purposes:
  1. `sandwich_collections` - Operational daily collection log for volunteers entering individual sandwich drops (446 records for 2023, 396k total)
  2. `authoritative_weekly_collections` - Scott's authoritative weekly aggregated data from Excel tracking system (422 records for 2023, 489k total) - **SOURCE OF TRUTH for analytics/reporting**
Authoritative Data Import: Imported 1,675 weekly collection records from "New Sandwich Totals Scott (5)" Excel file (Oct 30, 2025). Correct annual totals: 2020 (44,958), 2021 (281,383), 2022 (434,825), 2023 (489,186), 2024 (465,242), 2025 (288,838 through Aug 6). Import script: server/scripts/import-scott-simple.ts.
Hybrid Analytics System: Created `/api/collections/hybrid-stats` endpoint that intelligently combines both data sources. Uses Scott's authoritative data for 2020-2024 (complete) and 2025 through August 6, then switches to the collection log for dates after August 6, 2025 (when Scott's spreadsheet stopped being updated). This ensures analytics always show correct official totals while incorporating the latest volunteer-entered data.
Analytics Pages Updated (Oct 30, 2025): Both grant-metrics.tsx and impact-dashboard.tsx now fetch and use hybrid stats for yearly totals AND total impact instead of calculating from collection log. This fixes the 265k+ missing sandwiches issue in analytics displays. Pages fetch `/api/collections/hybrid-stats` and use the `total` field for overall impact and `byYear` object for authoritative yearly sandwich counts (2020-2025). Fallback to collection log calculation only if hybrid stats unavailable.
Collection Data Structure: Individual sandwiches are collected Wed/Thurs when people drop off at host homes. Group events are separate organized sandwich-making events throughout the week at various organizations. "Groups" host entries represent legitimate group collection events, NOT aggregates of individual entries.
