...
- **Twilio "EIO: i/o error, read" 500 on `/api/users/sms-opt-in`**: this is NOT a Twilio API/credentials error and NOT a code bug. The Twilio SDK lazy-`require()`s its resource tree at first use (`client.messages.create`); the stack shows `readFileSync` of `node_modules/twilio/lib/rest/api/v2010/account/availablePhoneNumberCountry.js` failing with EIO = a **corrupted/unreadable package file in the deployed image** (dev loads Twilio fine). **Fix: republish for a fresh deploy image; if it recurs across clean republishes, reinstall the `twilio` package.** Note: the opt-in route (`server/routes/sms-users.ts`) sends the confirmation SMS FIRST and only persists `metadata.smsConsent` after it succeeds — so a Twilio send failure blocks the opt-in entirely (by design: no code = nothing to confirm).
