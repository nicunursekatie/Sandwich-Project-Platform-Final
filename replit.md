...
- **Groups Catalog Event Count Fix**: Fixed incorrect event count display in groups catalog (`server/routes/groups-catalog.ts`) by changing department aggregation key from `${canonicalOrgName}|${department}|${request.id}` to `${canonicalOrgName}|${department}|${contactName}`. Previously, each event request created a separate department entry (causing 5 entries for 5 events), but only ONE entry received the `actualEventCount` from sandwich collections, resulting in cards showing "0 events" while the detail dialog correctly showed 5 events. Now events are properly aggregated by organization+department+contact, ensuring accurate event counts are displayed.
