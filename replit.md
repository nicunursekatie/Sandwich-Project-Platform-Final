...
- **Durable fix (RECOMMENDED — chosen "Option A", not yet implemented):** configure the session-store pg Pool with idle/connection/statement timeouts + TCP `keepAlive` + a pool `error` handler, so dead Neon sockets fail fast and a pool error can never wedge the process. Sessions stay in Postgres (users stay logged in across restarts). Rejected alternatives: switching the session store to the `@neondatabase/serverless` WebSocket Pool (more change, no real gain on a VM); moving sessions to Redis/in-memory (new infra, or logs everyone out on restart).
