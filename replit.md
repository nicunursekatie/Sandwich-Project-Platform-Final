...
- **Kudos Mark-as-Read System**: Kudos messages are tracked in `kudosTracking` table with `messageId` referencing the `messages` table. When marking kudos as read via `PATCH /api/emails/:id`, the system looks up `kudosTracking.messageId` (not `kudosTracking.id`) because kudos returned to the client have `id = messages.id`. Kudos use the messaging service (`messageRecipients` table) to mark as read, not the email service (`emailMessages` table).
