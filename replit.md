...
- **My Assignments Count Fix**: The My Assignments tab badge now accurately displays only actionable events (excludes completed and declined events from the count). Previously, the badge counted all assigned events but the tab filtered out completed/declined, causing confusing mismatches between badge count and visible events. Fixed in `EventRequestContext.tsx` by filtering `completed` and `declined` statuses from the count calculation to match the display logic.
- **Groups Catalog Organization Grouping**: Organizations in the groups catalog now properly consolidate all events into a single organization card. Previously, each event request created a separate duplicate card. Fixed in `groups-catalog.ts` by removing `request.id` from the aggregation key, allowing proper grouping by canonical organization name and department.
- **Weekly Monitoring Email Routing**: Fixed location-to-contact email routing to send reminders to actual host contacts instead of admin spam folder. Updated location names to match database (Sandy Springs/Chastain, Intown/Druid Hills full name, UGA instead of Collective Learning) and added fallback email mapping: Alpharetta→Nancy Miller, East Cobb/Roswell→Vicki Tropauer, Intown→Laura Baldwin, Flowery Branch→Kristina McCarthney, Dunwoody→Marcy Louza (primary) or Lisa/Steph when missing. System prioritizes fallback mapping over database lookup for reliability. Emails sent from verified domain `katie@thesandwichproject.org` to avoid spam filters. Template updated to friendly language about "sandwich collection numbers" with warm tone and clear app link.
