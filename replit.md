...
- **Groups Catalog Individual Event Display**: Each event request now displays as its own card within organization groups (`server/routes/groups-catalog.ts`). The aggregation key uses `${canonicalOrgName}|${department}|${contactName}|${request.id}` to ensure each event gets a separate card. Each card shows `actualEventCount = 1` for completed events and `0` for pending events, providing accurate per-event tracking rather than organization-wide counts. This allows users to see all individual events within an organization, not just department-level summaries.
