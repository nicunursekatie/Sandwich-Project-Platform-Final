...
- **Team Assignment Auto-Adjustment**: Implemented intelligent auto-adjustment logic for event team assignments. When drivers, speakers, or volunteers are assigned to an event, the system automatically ensures the corresponding "needed" fields (driversNeeded, speakersNeeded, volunteersNeeded) reflect the actual assignments. If assignments exceed current needs, the system automatically increases the needed count to match. This prevents impossible states like "1/0 assigned" and ensures data consistency. Logic implemented in PATCH /:id, PUT /:id, and PATCH /:id/drivers endpoints with null-safe handling for speakerDetails objects.
