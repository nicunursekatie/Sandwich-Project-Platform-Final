...
- **Confidential Documents Permission System**: Replaced hardcoded email whitelist with permission-based access control. The DOCUMENTS permission group in `shared/permission-config.ts` includes five permissions: `DOCUMENTS_VIEW`, `DOCUMENTS_MANAGE`, `DOCUMENTS_CONFIDENTIAL` (access to upload/view confidential documents), `DOCUMENTS_UPLOAD`, and `DOCUMENTS_DELETE_ALL`. Backend endpoint in `server/routes/storage/index.ts` now validates `user.permissions.includes('DOCUMENTS_CONFIDENTIAL')` instead of checking against a hardcoded email array. This makes confidential document access controllable via the permissions editor dialog.
- **Guided Tour UX Improvements (October 2025)**: Enhanced tutorial experience by reducing overlay dimming from 60% to 30% opacity (both `bg-black/30` overlay and `rgba(0,0,0,0.3)` spotlight shadow) for better visibility of highlighted elements. Improved scrolling behavior with hybrid approach: `scrollIntoView({ block: 'center', inline: 'nearest' })` provides stable vertical centering on long pages while preventing unwanted horizontal jumps. Changes apply globally to all tutorials via `client/src/components/GuidedTour.tsx`.
- **Google Sheets Sync System (October 2025)**: Event requests sync uses dual-layer duplicate prevention: (1) `imported_external_ids` table (permanent blacklist) prevents re-importing previously deleted events, and (2) unique constraints on `event_requests.external_id` prevent duplicate active events. When sync shows "blocked by permanent blacklist", it includes BOTH blacklist blocks AND unique constraint failures. Clearing the blacklist table allows re-import of previously deleted events, but existing active events cannot be re-imported due to unique constraints (working as designed for import-once guarantee). Auto-transition system (`server/background-sync-service.ts`) automatically moves scheduled events to completed status the day after their event date passes. Debug logging added (October 20, 2025) to track which events are checked and why past-due events might not transition. Manually transitioned events 654, 19, and 57 from Sept 30/Oct 1 that were not auto-transitioning (root cause under investigation - these events appear to not be returned by `getAllEventRequests()` despite existing in database with status='scheduled').
