...
impactReportsRouter.post('/generate-pdf', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    doc.text(sanitizePdfText(report.title), 20, 25);
    ...
    const summaryLines = doc.splitTextToSize(sanitizePdfText(report.executiveSummary), 170);
    ...
    if (report.highlights && report.highlights.length > 0) {
      ...
      report.highlights.forEach((highlight, index) => {
        ...
        doc.setFontSize(11);
        doc.setFont('helvetica', 'bold');
        doc.text(`${index + 1}. ${sanitizePdfText(highlight.title)}`, 25, yPosition + 2);

        doc.setTextColor(...darkGray);
        doc.setFontSize(9);
        doc.setFont('helvetica', 'normal');
        const descLines = doc.splitTextToSize(sanitizePdfText(highlight.description), 160);
        doc.text(descLines[0], 25, yPosition + 10);

        if (highlight.metric) {
          doc.setTextColor(...primaryColor);
          doc.setFont('helvetica', 'bold');
          doc.text(sanitizePdfText(highlight.metric), 165, yPosition + 2, { align: 'right' });
        }
    ...
    if (report.trends && report.trends.length > 0) {
      ...
      report.trends.forEach((trend) => {
        ...
        const label = trendLabels[trend.category] || 'Insight';
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');

        const trendText = `${label}: ${sanitizePdfText(trend.description)}`;
  ...
  try {
    ...
    events.forEach(e => {
      ...
      const sandwichCount = linkedCollection
        ...
        : getReportableSandwichCount(e, { ignoreSuspiciousEstimatedCounts: true });
