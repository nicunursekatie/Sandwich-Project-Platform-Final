...
documentsRouter.post('/',
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      logger.info(`Document upload by ${user.email}: "${title}" (${req.file.originalname})`);
...
documentsRouter.get('/:id/download', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = getUser(req);
    ...
    logger.info(`Document download attempt by ${user.email}: document ID ${documentId}`);
    ...
    if (!document) {
      logger.warn(`Document not found: ID ${documentId} requested by ${user.email}`);
      return res.status(404).json({ error: 'Document not found' });
    }

    // Check if document is active
    if (!document.isActive) {
      logger.warn(`Inactive document access attempt: ID ${documentId} by ${user.email}`);
    ...
    logger.info(`Document download success: ${user.email} downloaded document ID ${documentId} - "${document.originalName}"`);
...
documentsRouter.delete('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    logger.info(`Document delete attempt by ${user.email}: document ID ${documentId}`);
    ...
    if (!document) {
      logger.warn(`Document not found for deletion: ID ${documentId} by ${user.email}`);
      return res.status(404).json({ error: 'Document not found' });
    }

    // Check if user has permission to delete (uploader or admin)
    if (document.uploadedBy !== user.id && user.role !== 'admin') {
      logger.warn(`Unauthorized delete attempt: ${user.email} tried to delete document ID ${documentId}`);
    ...
    logger.info(`Document deleted: ${user.email} deleted document ID ${documentId} - "${document.title}"`);
...
documentsRouter.get('/:id/permissions', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = getUser(req);
...
documentsRouter.post('/:id/permissions', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = getUser(req);
...
documentsRouter.delete('/:id/permissions/:userId/:permissionType', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = getUser(req);
...
documentsRouter.get('/:id/access-logs', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const user = getUser(req);
