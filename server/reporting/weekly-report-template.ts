...
import { format, addDays, subDays, startOfWeek, endOfWeek, subWeeks } from 'date-fns';
...
export class WeeklyReportTemplate {
  ...
  async generateWeeklyReport(targetDate: string = format(new Date(), 'yyyy-MM-dd')): Promise<WeeklyReportData> {
    ...
    console.log(`Generating weekly report for week: ${format(weekStart, 'yyyy-MM-dd')} to ${format(weekEnd, 'yyyy-MM-dd')}`);
    ...
    const locationPerformance = await this.analyzeLocationPerformance(currentWeekData.locations, lastWeekData.locations);
    ...
    const celebratingSuccess = await this.getCelebratingSuccess(currentWeekData);
    ...
    const report: WeeklyReportData = {
      ...
      summary: {
        ...
        participation_rate: currentWeekData.totalLocations > 0 ? currentWeekData.activeLocations / currentWeekData.totalLocations : 0,
  ...
  private async getWeekData(startDate: Date, endDate: Date) {
    try {
      ...
      weekCollections.forEach(collection => {
        ...
        const group = (collection.groupCollections || []).reduce((sum: number, gc: any) => sum + (gc.count || 0), 0);
  ...
  private async getFourWeekAverages(endDate: Date) {
    ...
    const avgPerLocation = weeks.reduce((sum, week) => {
      return sum + (week.activeLocations > 0 ? week.total / week.activeLocations : 0);
    }, 0) / weeks.length;
  ...
  private async analyzeLocationPerformance(currentLocations: any[], lastWeekLocations: any[]) {
    const high_performers = [];
    const needs_attention = [];
    const steady_contributors = [];

    for (const location of currentLocations) {
      const lastWeekLocation = lastWeekLocations.find(l => l.name === location.name);
      const trend = this.calculateTrend(location.total, lastWeekLocation?.total || 0);
      ...
      } else if (location.total === 0 || this.isDecliningSeveralWeeks(location.name)) {
        const issues = [];
        if (location.total === 0) issues.push('No collections this week');
        if (this.isDecliningSeveralWeeks(location.name)) issues.push('Declining for 3+ weeks');
  ...
  private async isDecliningSeveralWeeks(locationName: string): Promise<boolean> {
  ...
  private async getNextWeekPreparation(endDate: Date) {
    ...
    return {
      host_confirmations: {
        ...
        percentage: activeHosts.length > 0 ? confirmedCount / activeHosts.length : 0
      },
      pending_actions: [
      ...
      collection_day_prep: {
        weather_forecast: 'Partly cloudy, 72°F - Good conditions for collection',
  ...
  private async getCelebratingSuccess(weekData: any) {
    ...
    return {
      ...
      volunteer_spotlight: {
        ...
        contribution: 'Organized 3 new group collections this month, resulting in 450 additional sandwiches'
      },
      impact_story: {
        quote: 'The sandwiches you provide help us serve 200+ families each week. Your consistency means everything to our community.',
