...
async function importEvents() {
  try {
    ...
    events.slice(0, 3).forEach((event, index) => {
      console.log(`${index + 1}. ${event.firstName} ${event.lastName} (${event.email}) - ${event.organizationName} - ${event.desiredEventDate ? event.desiredEventDate.toDateString() : 'No date'}`);
    });
    
    // Import into database
    console.log('\nInserting into database...');
    const result = await db.insert(eventRequests).values(events).returning({ id: eventRequests.id });
    
    console.log(`✅ Successfully imported ${result.length} events!`);
    console.log('Event IDs:', result.map(r => r.id));
