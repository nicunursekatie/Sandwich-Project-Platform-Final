...
async function fileToBase64(filePath: string): Promise<{ base64: string; mediaType: ImageMediaType }> {
  ...
  switch (ext) {
    ...
    case '.webp':
      ...
      break;
    default:
      // Default to jpeg for unknown types (like HEIC converted)
      mediaType = 'image/jpeg';
  }
...
OUTPUT FORMAT (JSON):
{
  ...
  "rawExtraction": "Brief description of what you see on the sheet"
    ...
    const userPrompt = contextHint
      ? `Please analyze this sign-in sheet photo and extract all sandwich collection data. Context: ${contextHint}`
    ...
    const entries: ExtractedCollectionEntry[] = (parsed.entries || []).map((entry: any) => ({
      ...
      sandwichCount: Math.max(0, parseInt(entry.sandwichCount) || 0),
